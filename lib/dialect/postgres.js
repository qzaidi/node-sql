...
Postgres.prototype.visit = function(node) {
  switch(node.type) {
    ...
    case 'USE INDEX'       : return this.visitUseIndex(node);
...
Postgres.prototype.visitUseIndex = function(index) {
  throw new Error("Unsupported use index hint in postgres ");
  var result = ['USE INDEX', index.nodes.map(this.visit.bind(this)).join(', ')];
  return result;
};
